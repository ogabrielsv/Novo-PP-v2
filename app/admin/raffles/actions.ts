...
export async function updateRaffle(id: string, formData: FormData) {
    ...
    const showProgressBar = formData.get('showProgressBar') === 'on'
    const progressBarValueRaw = formData.get('progressBarValue') as string
    const progressBarValue = Math.max(0, Math.min(100, parseInt(progressBarValueRaw) || 0))
