                        ...
                        {formData.showProgressBar && (
                            <div className="flex items-center gap-4 pl-8">
                                <div className="flex-1 space-y-1">
                                    <label className="text-xs text-stone-500 block">Porcentagem da barra</label>
                                    <input
                                        type="number"
                                        min={0}
                                        max={100}
                                        value={formData.progressBarValue}
                                        onChange={(e) => {
                                            const val = Math.max(0, Math.min(100, parseInt(e.target.value) || 0));
                                            setFormData({ ...formData, progressBarValue: val });
                                        }}
                                        className="w-full bg-stone-900 border border-stone-800 text-white rounded-lg py-2.5 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent transition-all"
                                    />
                                </div>
                                <div className="bg-blue-600/10 border border-blue-500/20 rounded-lg px-3 py-2 text-blue-400 text-sm font-medium">
                                    Valor atual: {formData.progressBarValue}%
                                </div>
                            </div>
                        )}
                    </div>
